...
norm.values <- cbind(log(1 + 10^4 * subset(data[[1]], select = as.character(unique(pdfs$gene))) / data[[1]]$nCount_RNA),
                     log(1 + 10^4 * exp(subset(data[[1]], select = paste0(as.character(unique(pdfs$gene)), "_expression")))),
...
pdf.sanity <- do.call(rbind, lapply(intersect(as.character(sanity.values$gene), as.character(pdfs$gene)), function(my.gene){
  x.log <- seq(min(m.sanity[my.gene, ]) - 2 * max(m.sanity.e[my.gene, ]),
               log((exp(unique(my.table$xmax[my.table$gene == my.gene])) - 1) * 10^-4),
               length.out = 200)
  post <- get.post(x.log,
                   m.sanity[my.gene, ],
                   m.sanity.e[my.gene, ])
  dx.log <- x.log[2] - x.log[1]
  x.log.borders <- c(x.log - dx.log / 2, x.log[length(x.log)] + dx.log / 2)
  x.seurat.borders <- log(1 + 10^4 * exp(x.log.borders))
  x.seurat <- (x.seurat.borders[-1] + x.seurat.borders[-length(x.seurat.borders)]) / 2
  pdf.seurat <- post / (1 - exp(-x.seurat))
  dx.seurat <- x.seurat.borders[-1] - x.seurat.borders[-length(x.seurat.borders)]
  pdf.seurat <- pdf.seurat / sum(dx.seurat * pdf.seurat)
  return(data.frame(x = x.seurat, mean = pdf.seurat, group = "all", gene = my.gene))
}))
...
expected.values.df <- do.call(rbind, lapply(as.character(unique(pdfs$gene)), function(my.gene){
  x.log <- seq(min(m.sanity[my.gene, ]) - 2 * max(m.sanity.e[my.gene, ]),
               log((exp(unique(my.table$xmax[my.table$gene == my.gene])) - 1) * 10^-4),
               length.out = 200)
  val <- expected.values(my.gene, x.log)
  dx.log <- x.log[2] - x.log[1]
  x.log.borders <- c(x.log - dx.log / 2, x.log[length(x.log)] + dx.log / 2)
  x.seurat.borders <- log(1 + 10^4 * exp(x.log.borders))
  x.seurat <- (x.seurat.borders[-1] + x.seurat.borders[-length(x.seurat.borders)]) / 2
  pdf.seurat <- val / (1 - exp(-x.seurat))
  dx.seurat <- x.seurat.borders[-1] - x.seurat.borders[-length(x.seurat.borders)]
  pdf.seurat <- pdf.seurat / sum(dx.seurat * pdf.seurat) * (1 - meta.generation$prop.zero[meta.generation$gene == my.gene])
  return(data.frame(x = x.seurat, val = pdf.seurat, gene = my.gene))
}))
...
all.expected.values.df$gene <- factor(all.expected.values.df$gene, levels = meta.generation$gene)
...
g <- ggplot(my.df) +
  ...
  geom_density(data = subset(sanity.values, gene %in% my.df$gene),
               aes(x = log(1 + 10^4 * exp(value)), color = "Density from Sanity"), fill = NA,
               trim = T, key_glyph = "path")  +
...
# Plot only the group "all"
my.df <- subset(pdfs, group == "all")
g <- ggplot(my.df) +
  geom_ribbon(aes(x = x, ymin = low, ymax = high, fill = "baredSC"), alpha = 0.3, color = NA) +
...
ggsave(paste0(output.prefix, "_combinedModels_all_only_all_cells.pdf"),
       width = 3 + 2 * length(unique(my.df$gene)) / 2, height = 6, limitsize = F)

# Plot only the group "all" cut at 3
my.df <- subset(pdfs, group == "all")
g <- ggplot(my.df) +
  geom_ribbon(aes(x = x, ymin = low, ymax = high, fill = "baredSC"), alpha = 0.3, color = NA) +
  facet_wrap(. ~ gene, scales='free',
  ...
  ylab("Density")  +
  facetted_pos_scales(
    y = lapply(meta.generation$prop.zero, function(x){
      if (x == 0.25){
        return(scale_y_continuous(limits = c(0, 2.5)))
      } else if (x == 0.5){
        return(scale_y_continuous(limits = c(0, 3.5)))
      }})
  )

ggsave(paste0(output.prefix, "_combinedModels_all_only_all_cells_cut_ggh4x.pdf"),
       width = 3 + 2 * length(unique(my.df$gene)) / 2, height = 6, limitsize = F)

ggarrange(g, labels = c("B"))
ggsave(paste0(output.prefix, "_combinedModels_all_only_all_cells_cut_ggh4x_B.pdf"),
       width = 3 + 2 * length(unique(my.df$gene)) / 2, height = 6, limitsize = F)
