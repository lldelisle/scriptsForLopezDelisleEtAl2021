...
my.cor <- do.call(rbind, lapply(unique(pdfs$id), function(my.id){
  ...
  return(data.frame(id = my.id, corr = c(cor.pre, cor.input, cor.mcmc), condition = c("prePoisson", "postPoisson", "baredSC")))
}))
my.cor$condition <- factor(my.cor$condition, levels = c("prePoisson", "postPoisson", "baredSC"))
...
# Labeller:
my.labeller <- rep(c("mainly correlated", "mainly anti-correlated", "independent",
                 "mainly correlated\nsmaller scale for g1", "mainly anti-correlated\nsmaller scale for g1", "independent\nsmaller scale for g1",
                 "2 populations\ncorrelated", "2 populations\nanti-correlated",
                 "each gene\nexpressed in half\nindependent", "each gene\nexpressed in half\nmainly anti-correlated", "each gene\nexpressed in half\nfully anti-correlated"),
                 each = 2)
names(my.labeller) <- as.character(1:length(my.labeller))
labeller.cond <- c("generated\nexpression", "simulated\nscRNA-seq\nnormalized counts",
                   "expression\ninfered\nby baredSC")
names(labeller.cond) <- c("prePoisson", "postPoisson", "baredSC")
...
all.pdfs <- rbind(pdfs[, c("x", "y", "value", "id", "condition")], pdfs.pre, pdfs.input)
all.pdfs <- merge(all.pdfs, unique(meta.data[, c("i", "id", "group")]))
all.pdfs$condition <- factor(all.pdfs$condition, levels = c("prePoisson", "postPoisson", "baredSC"))
...
g <- ggplot(all.pdfs_round, aes(x, y)) +
  ...
  scale_fill_gradient(low="white", high="black") 
ggsave(paste0(output.prefix, "_combinedModels_all.pdf"), g, width = 3 + 1.5 * length(unique(all.pdfs_round$id)), height = 5, limitsize = F)

# Selected experiments to plot:
figs <- list('fig3a' = 1:12,
             'fig3b' = 13:16,
             'fig3c' = 17:22
)
# Get the size of each group
...
for(fig.name in names(figs)){
  # Main:
  my.df <- subset(all.pdfs_round, i %in% figs[[fig.name]] & group == "all")
  my.corr <- subset(corr, i %in% figs[[fig.name]] & group == "all")
  my.cor.from.pdf <- subset(my.cor, i %in% figs[[fig.name]] & group == "all")
  ...
  g <- ggplot(my.df, aes(x, y)) +
    ...
    scale_fill_gradient(low="white", high="black")
  if (fig.name != 'fig3c'){
    ggplots[[gsub("fig", "figS", fig.name)]] <- g +
      theme(legend.position = "none")
  } else {
    ggplots[[gsub("fig", "figS", fig.name)]] <- g
  }
  ggsave(paste0(output.prefix, "_combinedModels_", gsub("fig", "figS", fig.name), ".pdf"), g,
         width = 3 + 1.5 * length(unique(my.df$id)), height = 5, limitsize = F)
}

g.all <- ggarrange(
  ggarrange(ggplots[['fig3a']],
            labels = c("A")),
  ggarrange(ggplots[['fig3b']], ggplots[['fig3c']],
            labels = c("B", "C"), widths = c(1.2, 2)),
  nrow = 2
)
ggsave(paste0(output.prefix, "_combinedModels_fig3abc.pdf"), g.all,
       #        width = 15, height = 12)
       width = 10, height = 9)

g.all.s <- ggarrange(
  ggarrange(ggplots[['figS3a']],
            labels = c("A")),
  ggarrange(ggplots[['figS3b']], ggplots[['figS3c']],
            labels = c("B", "C"), widths = c(1.2, 2)),
  nrow = 2
)
ggsave(paste0(output.prefix, "_combinedModels_figS3abc.pdf"), g.all.s,
       #        width = 15, height = 12)
       width = 19, height = 11)
